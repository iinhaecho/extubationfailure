...
def init_class_columns(group):
    ...
    1. mvtime_state: 삽관발관 시간 차에 따라 mvtime_state True 마킹
...
def flag_mvtime_state(group):
    """
    mvtime이 1440분 이내 (<= 24시간)이면 mechanical ventilation (True)으로 분류합니다. 
    """

    group.loc[group['mvtime'] <= 1440, 'mvtime_state'] = True
...
def fill_class_columns(group):
    ...
    - 삽관-발관 시간차이가 24시간 이내일 경우, mechanical ventilation으로 규정 (flag_mvtime_state).
